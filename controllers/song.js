...
function getSong(req,res){
	...
	Song.findById(songId).populate({path: 'album'}).exec((err, song) => {
		if(err){
			res.status(500).send({message: 'Error getting data from database'});
		}else{
			if(!song){
				res.status(404).send({message: 'Song doens\'t exist'});
			}else{
...
function updateSong(req,res){
	...
	Song.findByIdAndUpdate(songId, update, (err,songUpdated) =>{
		if(err){
			...
			if(!songUpdated){
				res.status(404).send({message: 'The song wasn\'t updated'});
			}else{
...
function deleteSong(req,res){
	...
	Song.findByIdAndDelete(songId,(err, songRemoved) =>{
		if(err){
			...
			if(!songRemoved){
				res.status(404).send({message: 'The song wasn\'t deleted'});
			}else{
	...
	if(req.files){
		...
		if(file_ext == 'mp3' || file_ext == 'ogg' ){
			...
			Song.findByIdAndUpdate(songId, {file: file_name}, (err,songUpdated) => {
				if(!songUpdated){
					res.status(404).send({message: "Something went wrong. Song wasn't updated."});
				}else{
...
function getSongFile(req,res){
	...
	fs.exists(path_file, function(exists){
		if(exists){
			...
			res.status(200).send({message: 'Song doesn\'t exist'});
		}
	});
}
module.exports = {
